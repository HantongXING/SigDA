...
for epoch in range(1600):  
    ...
    if precision > precesion_previous:
        ...
        torch.save(model, '/data/xht/modulation_recog/open-source-code/comparative_experiment/checkpoints/cnn2.pth')
        for j in range(20):
            num = 0
            for k in range(11):
                num = num + cmt[k, k, j]
...
############################# Testing #############################################
net = torch.load('/data/xht/modulation_recog/open-source-code/comparative_experiment/checkpoints/cnn2.pth')
net.eval()
correct,fals = 0,0
cmt = torch.zeros(11, 11, 20, dtype=torch.int16)
for i, data in enumerate(test_loader_all):
