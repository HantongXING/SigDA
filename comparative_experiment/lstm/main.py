...
for epoch in range(1600):  
    ...
    for i, data in enumerate(train_loader_all):
        ...
        loss = criterion1(logits, train_label)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if precision > precesion_previous:
        ...
        torch.save(model, '/data/xht/modulation_recog/open-source-code/comparative_experiment/checkpoints/lstm.pth')
...
############################# Testing #############################################
net = torch.load('/data/xht/modulation_recog/open-source-code/comparative_experiment/checkpoints/lstm.pth')
net.eval()
correct = 0
fals = 0
cmt = torch.zeros(11, 11, 20, dtype=torch.int16)
