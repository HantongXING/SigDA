...
MS = torch.load('/data/xht/modulation_recog/open-source-code/checkpoints/source.pth')
MS.eval()
MT = torch.load('/data/xht/modulation_recog/open-source-code/checkpoints/source.pth')
print(MT)
...
for epoch in range(300):
    for i, (sdata, tdata) in enumerate(zip(train_loader_all, test_loader_all)):
        ...
        D_loss = criterion(discriminator(train_feature.detach()), l_zero.long().cuda()) + criterion(discriminator(test_feature.detach()), l_one.long().cuda())
        optimizerd.zero_grad()
        D_loss.backward()
        optimizerd.step()

        loss1 = criterion(discriminator(test_feature), l_zero.long().cuda())
        G_loss = loss1
    ...
    if precision>precesion_previous:
        best_epoch = epoch
        torch.save(MT, '/data/xht/modulation_recog/open-source-code/checkpoints/a2c_source.pth')
